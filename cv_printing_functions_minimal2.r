...
create_CV_object <-  function(data_location,
                              pdf_mode = FALSE,
                              short = FALSE, # nur markierte Einträge behalten
  ...
  cv <- list(
    ...
    # cv$entries_data <- readr::read_delim(paste0(data_location, "entries.csv"),         skip = 1, delim = ";")
    cv$entries_data <- readxl::read_xlsx(here("data","entries.xlsx"),sheet = 1, skip = 1,na = c("",NA)) 
    cv$skills       <- readr::read_delim(here("data","skills.csv"), skip = 1, delim = ";")
    cv$lang         <- readr::read_delim(here("data","language.csv"), skip = 1, delim = ";",locale = locale(encoding = "ISO-8859-1"))
    cv$text_blocks  <- readr::read_delim(here("data","text_blocks.csv"),     skip = 1, delim = ";",locale = locale(encoding = "ISO-8859-1"))
    cv$contact_info <- readr::read_delim(here("data","contact_info.csv"),    skip = 1, delim = ";",locale = locale(encoding = "UTF-8"))
  ...
  cv$entries_data %<>%
    { if(short ==T) dplyr::filter(.,in_resume == TRUE) else . } %>% 
    mutate(loc = ifelse(grepl("paper",section) & !is.na(link) ,paste0("<a href=",link,">",loc,"</a>"), loc),  # add links for published papers
           loc = ifelse(!is.na(hrsg),paste0(hrsg,": ",loc,". ",publ) ,loc),
           loc = ifelse(grepl("paper",section) & is.na(loc),"",loc),
           authors = ifelse(grepl("(paper|konf)",section),description_1,NA), # autoren-var für paper & conf -> 2. Zeile
           loc = ifelse(grepl("konf",section),paste0(loc,", ",institution),loc), # Konferenzort anfügen
           loc = ifelse(grepl("(donk|Arbeitsmarkt\\-)",loc),paste0(loc,", ",institution),loc), # Ort anfügen wenn Angabe fehlt
           institution = NA # suppress location marker
           ) %>%
    tidyr::unite(
    ...
    dplyr::mutate(
      ...
      start_my = ifelse(!is.na(start_m),paste0(str_pad(start_m,2,"left","0"),"/",start_year), start_year),
      end_my =   ifelse(!is.na(  end_m),paste0(str_pad(end_m,  2,"left","0"),"/",end_year),   end_year  ),
      ...
      timeline = dplyr::case_when(
        no_start  & no_end  ~ "N/A",
        no_start  & has_end ~ as.character(end_my),
        has_start & no_end  ~ paste("Bis heute", "-", start_my),
        TRUE                ~ paste(end_my, "-", start_my)
      )
      
    ) %>%
    dplyr::arrange(desc(parse_dates(end)),desc(end_m)) %>%
...
sanitize_links <- function(cv, text){
  if(cv$pdf_mode){
    link_titles <-       text # stringr::str_extract_all(text, '(?<=\\[).+?(?=\\])')[[1]]
    link_destinations <- text # stringr::str_extract_all(text, '(?<=\\().+?(?=\\))')[[1]]
...
#' @description special print for paper section to include links have different heading structure.
#' @param section_id ID of the entries section to be printed as encoded by the `section` column of the `entries` table
# <a href={link}>{loc}</a> 
print_paper <- function(cv, section_id, glue_template = "default"){
...
#' @description Construct a bar chart of language, copied from skill_bars
print_lang_bars <- function(cv, out_of = 5, bar_color = "#969696", bar_background = "#C9C9C9", glue_template = "default"){
...
>{skill}</div>"
  ...
  cv$lang %>%
    dplyr::mutate(width_percent = round(100*as.numeric(level)/out_of)) %>%
    dplyr::mutate(skill = paste0(skill, " (",note,")")) %>% 
    glue::glue_data(glue_template) %>%
    print()
